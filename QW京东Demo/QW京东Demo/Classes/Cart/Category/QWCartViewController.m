...
#import "QWCartViewController.h"
#import "QWProductToolCommon.h"
#import "QWCartTableCell.h"
#import "QWCartToolbar.h"
...
@interface QWCartViewController ()  <QWCartToolbarDelegate, QWCartTableCellDelegate,
QWCartEditToolbarDelegate, UIAlertViewDelegate>

// 工具条
@property (nonatomic, weak) QWCartToolbar *toolBar;
@property (nonatomic, weak) QWCartEditToolBar *editToolBar;
// 购物车中的商品信息
...
@implementation QWCartViewController

#pragma mark - 懒加载
- (NSMutableArray *)products {
    if (_products == nil) {
        _products = [QWCartTool cart];
...
- (NSMutableArray *)cellItems {
    if (_cellItems == nil) {
        // 初始时为全部选中状态
        _cellItems = [QWCellSelectionItem selectAllCellItems];
    }
    return _cellItems;
}

- (QWCartEditToolBar *)editToolBar {
    if (_editToolBar == nil) {
        QWCartEditToolBar *editToolBar = [[[NSBundle mainBundle] loadNibNamed:@"QWCartEditToolBar" owner:nil options:nil] lastObject];
...
- (void)setSelectAll:(BOOL)selectAll {
    ...
    if (selectAll) {            // 如果全部选中
        ...
        _cellItems = [QWCellSelectionItem deselectAllCellItems];
    }
}
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    // 修改tableView的frame
    self.tableView.frame = CGRectMake(self.tableView.x,
                                      self.tableView.y,
                                      self.tableView.width,
                                      self.tableView.height - QWBuyerToolBarHeight);
}
...
- (void)addToolBar {
    ...
    CGFloat X = 0;
    CGFloat Y = self.tableView.superview.height - height - self.tabBarController.tabBar.height;
    
    // 添加工具条
    QWCartToolbar *toolBar = [[QWCartToolbar alloc] init];
...
- (void)tableViewCell:(QWCartTableCell *)cell atIndexPath:(NSIndexPath *)indexPath didClickButtonAtIndex:(NSInteger)index {
    if (index == 0) {
        // 切换cell选中状态
        [self switchSelectedStateForCell:cell AtIndexPath:indexPath];
    } else {
        // 删除1件商品
        [QWCartTool buyMoreProduct:self.products[indexPath.row] count:(index - 1) ? @"1" : @"-1"];
        // 刷新页当前面数据
        [self reloadData:YES fromFile:YES];
    }
}
...
- (void)switchSelectedStateForCell:(QWCartTableCell *)cell AtIndexPath:(NSIndexPath *)indexPath {
    // 切换cell选中状态
    NSString *cellState = [self.cellItems[indexPath.row] boolValue] ? QW_NO_string : QW_YES_string;
...
- (void)postNotificationToTabBarVC {
    [[NSNotificationCenter defaultCenter] postNotificationName:QWProductBuyCountDidChangeNotification object:nil];
}
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
    if (buttonIndex == 1) {
        [self.cellItems enumerateObjectsUsingBlock:^(NSString *obj, NSUInteger idx, BOOL *stop) {
            if (obj.boolValue) {
                QWProductInfo *product = self.products[idx];
                [QWCartTool removeProductWithSkuId:product.skuId];
            }
        }];
        self.selectAll = NO;
        [self reloadData:YES fromFile:YES];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    QWCartTableCell *cell = [QWCartTableCell cellWithTableView:tableView];
    // 设置cell
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    // 取出模型
    QWProductInfo *selectedProductInfo = self.products[indexPath.row];
    // 页面跳转
    //    self.hidesBottomBarWhenPushed = YES;
    QWProductMainController *detailVC
    = [[QWProductMainController alloc] initWithSkuId:selectedProductInfo.skuId];
