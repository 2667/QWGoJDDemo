...
#define QWItemKeyPathFocused        @"focused"
#define QWItemKeyPathProductCount   @"productCount"

@interface QWDetailBarButton ()
...
@implementation QWDetailBarButton
...
+ (instancetype)buttonWithTag:(NSUInteger)tag {
    QWDetailBarButton *button = [self buttonWithType:UIButtonTypeCustom];
...
- (void)setItem:(QWMyProductItem *)item {
    ...
    [item addObserver:self
           forKeyPath:QWItemKeyPathFocused
              options:NSKeyValueObservingOptionNew
              context:nil];
    [item addObserver:self
           forKeyPath:QWItemKeyPathProductCount
              options:NSKeyValueObservingOptionNew
              context:nil];
}
...
- (void)observeValueForKeyPath:(NSString *)keyPath
    ...
    if (self.tag == 1) {
        ...
        if ([_item.isFocused isEqualToString:QW_YES_string]) {
...
- (void)layoutSubviews {
    ...
    CGFloat imageHeight = self.bounds.size.height * QWImageRidio; //  0.7倍高度
    ...
    // 3. badgeView
    self.badgeView.x = self.width * 0.5 + 6;
    self.badgeView.y = 5;
...
- (void)dealloc {
    ...
    [_item removeObserver:self forKeyPath:QWItemKeyPathFocused];
    [_item removeObserver:self forKeyPath:QWItemKeyPathProductCount];
}
