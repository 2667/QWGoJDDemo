...
#define QWAuthorizeURL      @"https://oauth.jd.com/oauth/authorize"
...
- (void)viewDidLoad {
    ...
    // 2. 请求获取用户授权码code
    NSURLRequest *request = [self creatRequestOfAuthorizeByBaseURL:QWAuthorizeURL
                                                          clientID:QWClient_id
                                                       redirectURI:QWRedirect_uri];
...
- (void)accessTokenByCode:(NSString *)code
{
    ...
    [QWAccountTool accountWithCode:code success:^{  // 获取成功时调用
        
        // 进入tabBarVC
        QWTabBarController *tabBarVC = [[QWTabBarController alloc] init];
        
        // 切换keyWindow的根控制器：可以直接把之前的根控制器覆盖掉
        QWKeyWindow.rootViewController = tabBarVC;
